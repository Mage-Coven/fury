...
func getCmdMintDerivative() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s mint furyvaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd 10000000ufury --from <key>`, version.AppName, types.ModuleName,
		),
...
func getCmdBurnDerivative() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s burn 10000000bfury-furyvaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9z59jqd --from <key>`, version.AppName, types.ModuleName,
		),
