...
import (
	...
	"github.com/mage-coven/fury/x/community/client/utils"
	"github.com/mage-coven/fury/x/community/types"
)
...
func getCmdFundCommunityPool() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s fund-community-module 10000000ufury --from <key>`, version.AppName, types.ModuleName,
		),
...
Example:
$ %s tx gov submit-proposal community-pool-lend-deposit <path/to/proposal.json> --deposit 1000000000ufury --from=<key_or_address>
...
Note that --deposit below is the initial proposal deposit submitted along with the proposal.
Example:
$ %s tx gov submit-proposal community-pool-lend-withdraw <path/to/proposal.json> --deposit 1000000000ufury --from=<key_or_address>
Where proposal.json contains:
{
