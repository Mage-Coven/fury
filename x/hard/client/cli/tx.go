...
%[1]s tx %[2]s repay 1000000000ufury,25000000000bnb --from <key>
%[1]s tx %[2]s repay 1000000000ufury,25000000000bnb --owner <owner-address> --from <key>`, version.AppName, types.ModuleName),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
...
func getCmdLiquidate() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Example: fmt.Sprintf(
			`%s tx %s liquidate fury1hgcfsuwc889wtdmt8pjy7qffua9dd2tralu64j --from <key>`, version.AppName, types.ModuleName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
