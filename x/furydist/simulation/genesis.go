...
// RandomizedGenState generates a random GenesisState for furydist module
func RandomizedGenState(simState *module.SimulationState) {
	...
	furydistGenesis := types.NewGenesisState(params, types.DefaultPreviousBlockTime)
	if err := furydistGenesis.Validate(); err != nil {
		panic(err)
	}

	bz, err := json.MarshalIndent(&furydistGenesis, "", " ")
	if err != nil {
		panic(err)
	}
	fmt.Printf("Selected randomly generated %s parameters:\n%s\n", types.ModuleName, bz)
	simState.GenState[types.ModuleName] = simState.Cdc.MustMarshalJSON(furydistGenesis)
}
