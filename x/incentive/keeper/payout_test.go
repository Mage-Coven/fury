...
import (
	...
	"github.com/mage-coven/fury/x/incentive/keeper"
	"github.com/mage-coven/fury/x/incentive/testutil"
	"github.com/mage-coven/fury/x/incentive/types"
	...
	tests := testCases{
		{
			...
			args: args{
				accArgs: accountArgs{
					periods: []vestingtypes.Period{
						{Length: 5, Amount: cs(c("ufury", 5))},
						{Length: 5, Amount: cs(c("ufury", 5))},
						{Length: 5, Amount: cs(c("ufury", 5))},
						{Length: 5, Amount: cs(c("ufury", 5))},
					},
					origVestingCoins: cs(c("ufury", 20)),
					startTime:        100,
					endTime:          120,
				},
				period:              vestingtypes.Period{Length: 2, Amount: cs(c("ufury", 6))},
				ctxTime:             time.Unix(101, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 3, Amount: cs(c("ufury", 6))},
					{Length: 2, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
				},
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 7, Amount: cs(c("ufury", 6))},
				ctxTime:             time.Unix(80, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 7, Amount: cs(c("ufury", 6))},
					{Length: 18, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
				},
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 5, Amount: cs(c("ufury", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 5, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 5))},
					{Length: 5, Amount: cs(c("ufury", 11))},
					{Length: 5, Amount: cs(c("ufury", 5))},
				},
				expectedStartTime: 100,
				expectedEndTime:   120,
		...
		},
		{
			...
			args: args{
				...
				period:              vestingtypes.Period{Length: 50, Amount: cs(c("ufury", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: []vestingtypes.Period{
					{Length: 5, Amount: cs(c("ufury", 5))},
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			...
			if tc.args.mintModAccountCoins {
				authBuilder = authBuilder.WithSimpleModuleAccount(furydisttypes.ModuleName, tc.args.period.Amount)
			}
			...
			err := suite.keeper.SendTimeLockedCoinsToPeriodicVestingAccount(suite.ctx, furydisttypes.ModuleName, suite.addrs[0], tc.args.period.Amount, tc.args.period.Length)
...
func (suite *PayoutTestSuite) TestSendCoinsToBaseAccount() {
	authBuilder := app.NewAuthBankGenesisBuilder().
		WithSimpleAccount(suite.addrs[1], cs(c("ufury", 400))).
		WithSimpleModuleAccount(furydisttypes.ModuleName, cs(c("ufury", 600)))
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, furydisttypes.ModuleName, suite.addrs[1], cs(c("ufury", 100)), 5)
	suite.Require().NoError(err)
	acc := suite.getAccount(suite.addrs[1])
	vacc, ok := acc.(*vestingtypes.PeriodicVestingAccount)
	suite.True(ok)
	expectedPeriods := []vestingtypes.Period{
		{Length: int64(5), Amount: cs(c("ufury", 100))},
	}

	bk := suite.app.GetBankKeeper()

	suite.Equal(expectedPeriods, vacc.VestingPeriods)
	suite.Equal(cs(c("ufury", 100)), vacc.OriginalVesting)
	suite.Equal(cs(c("ufury", 500)), bk.GetAllBalances(suite.ctx, vacc.GetAddress()))
...
func (suite *PayoutTestSuite) TestSendCoinsToInvalidAccount() {
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, furydisttypes.ModuleName, suite.addrs[2], cs(c("ufury", 100)), 5)
	suite.Require().ErrorIs(err, types.ErrAccountNotFound)

	macc := suite.getModuleAccount(cdptypes.ModuleName)
	err = suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, furydisttypes.ModuleName, macc.GetAddress(), cs(c("ufury", 100)), 5)
