...
HTTPS_GIT := https://github.com/Mage-Coven/fury.git
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/mage-coven/fury
.PHONY: format
...
test-e2e: docker-build
	export E2E_FURY_FUNDED_ACCOUNT_MNEMONIC='tent fitness boat among census primary pipe nose dream glance cave turtle electric fabric jacket shaft easy myself genuine this sibling pulse word unfold'; \
